...
import { calculateTaxAmount, calculateIrpfAmount, calculateTotalAmount } from "@/lib/utils/invoice-calculations"
...
export function InvoiceForm({ invoice, onSubmit, onCancel }: InvoiceFormProps) {
  const [formData, setFormData] = useState<InvoiceFormData>(invoice || emptyInvoice)
  ...
  useEffect(() => {
    ...
    const irpfAmount = calculateIrpfAmount(formData.subtotal, formData.irpf_rate)
    const totalAmount = calculateTotalAmount(formData.subtotal, taxAmount, irpfAmount)
              ...
              <div className="grid gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input id="tax_amount" type="number" step="0.01" value={formData.tax_amount} disabled />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("irpf_rate", Number.parseFloat(e.target.value) || 0)}
                    />
                  </div>
                  <div className="grid gap-2">
                    <Label htmlFor="irpf_amount">Importe IRPF</Label>
                    <Input id="irpf_amount" type="number" step="0.01" value={formData.irpf_amount} disabled />
                  </div>
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="total_amount">Total</Label>
                  <Input id="total_amount" type="number" step="0.01" value={formData.total_amount} disabled />
                </div>
              </div>
            </div>
          </CardContent>
