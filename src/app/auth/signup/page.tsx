            ...
            {[
              ...
              <div key={index} className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/70">Lorem ipsum description.</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-gray-50">
        <div className="w-full max-w-sm space-y-8">
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            {error && <div className="p-4 rounded-lg bg-red-50 border border-red-100 text-red-600 text-sm">{error}</div>}
            {["Name", "Email", "Password", "Phone"].map((label, index) => (
              <div key={index} className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">{label}</label>
                <input
                  type={label === "Password" ? "password" : "text"}
                  value={label === "Name" ? name : label === "Email" ? email : label === "Password" ? password : phone}
                  onChange={(e) => {
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-2.5 px-4 rounded-lg text-white text-sm font-semibold ${isLoading ? "bg-blue-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 active:bg-blue-800"} transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 shadow-lg shadow-blue-600/20`}
            >
              {isLoading ? "Creating account..." : "Create account"}
            </button>
            <p className="text-center text-sm text-gray-500">
              Already have an account? <Link href="/auth/login" className="font-medium text-blue-600 hover:text-blue-500">Sign in</Link>
            </p>
          </form>
        </div>
      </div>
